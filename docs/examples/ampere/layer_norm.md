...
[:material-github: View on GitHub](https://github.com/patrick-toulme/pyptx/blob/dev/examples/ampere/layer_norm.py){ .md-button } 
[:material-file-code: `examples/ampere/layer_norm.py`](https://github.com/patrick-toulme/pyptx/blob/dev/examples/ampere/layer_norm.py){ .md-button }

## Overview

Ampere LayerNorm example using the maintained pyptx kernel path.

Run ``python examples/ampere/layer_norm.py`` to execute both a ``jax.jit``
path and a PyTorch eager path on ``sm_80`` (A100).

The kernel itself is the maintained ``examples/hopper/layer_norm.py`` —
mean + variance with v4 loads/stores. The only arch-specific bit is
the ``sm_80`` PTX target.
...
??? example "Full source"
    ...
    def build_layer_norm(B: int, N: int, *, eps: float = 1e-5, rows_per_cta: int | None = None):
        ...
        return _build_layer_norm(B, N, eps=eps, rows_per_cta=rows_per_cta, arch="sm_80")
