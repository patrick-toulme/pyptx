"""Double-buffered K-loop wgmma: separate SMEM per iter."""
...
def gemm(A, B, C):
    # Two independent buffers, one per iteration
    sA0 = smem.wgmma_tile(bf16, (64, 16), major="K")
    sA1 = smem.wgmma_tile(bf16, (64, 16), major="K")
    sB0 = smem.wgmma_tile(bf16, (16, 8),  major="MN")
    sB1 = smem.wgmma_tile(bf16, (16, 8),  major="MN")
    ...
    # Prefetch iter 0 into sA0/sB0
    with ptx.if_(tid == 0):
        ptx.mbarrier.arrive_expect_tx(bar0[0], 64*16*2 + 16*8*2)
        ptx.cp.async_.bulk.tensor_2d(dst=sA0[0], src=A.tma_desc(), coord=(0, 0), mbar=bar0[0])
        ptx.cp.async_.bulk.tensor_2d(dst=sB0[0], src=B.tma_desc(), coord=(0, 0), mbar=bar0[0])
    # Prefetch iter 1 into sA1/sB1 (different buffers!)
    with ptx.if_(tid == 0):
        ptx.mbarrier.arrive_expect_tx(bar1[0], 64*16*2 + 16*8*2)
        ptx.cp.async_.bulk.tensor_2d(dst=sA1[0], src=A.tma_desc(), coord=(16, 0), mbar=bar1[0])
        ptx.cp.async_.bulk.tensor_2d(dst=sB1[0], src=B.tma_desc(), coord=(0, 16), mbar=bar1[0])
    ptx.bar.sync(0)

    # Wait and consume iter 0
    ptx.mbarrier.wait(bar0[0], phase0)
    ptx.wgmma.fence()
    ptx.wgmma.mma_async(shape=(64, 8, 16), dtype_d=f32, dtype_a=bf16, dtype_b=bf16,
        d=acc, a=sA0, b=sB0, scale_d=False, trans_a=0, trans_b=1)
    ptx.wgmma.commit_group()
    ptx.wgmma.wait_group(0)

    # Wait and consume iter 1
    ptx.mbarrier.wait(bar1[0], phase1)
    ptx.wgmma.fence()
    ptx.wgmma.mma_async(shape=(64, 8, 16), dtype_d=f32, dtype_a=bf16, dtype_b=bf16,
        d=acc, a=sA1, b=sB1, scale_d=True, trans_a=0, trans_b=1)
    ptx.wgmma.commit_group()
    ptx.wgmma.wait_group(0)
