"""TMA into sA/sB -> wgmma -> TMA into DIFFERENT sC.
Does TMA writing to a different SMEM buffer after wgmma work?"""
...
def k(A, B, out):
    ...
    sC = smem.alloc(bf16, (64, 16), swizzle="32B")   # different buffer
    ...
    with ptx.if_(tid == 0):
        ptx.mbarrier.arrive_expect_tx(bar0[0], 2304)
    ...
    # TMA into sC (NOT sA/sB) - different SMEM buffer
    with ptx.if_(tid == 0):
        ptx.mbarrier.arrive_expect_tx(bar1[0], 64*16*2)
        ptx.cp.async_.bulk.tensor_2d(dst=sC, src=A.tma_desc(), coord=(0, 0), mbar=bar1[0])
    ptx.bar.sync(0)
    ptx.mbarrier.wait(bar1[0], phase1)

    # Store sC
    with ptx.if_(tid == 0):
        ptx.cp.async_.bulk.tensor_2d.store(dst=out.tma_desc(), src=sC, coord=(0, 0))
