"""K-loop with warpgroup-wide (count=128) release barrier."""
...
def gemm(A, B, C):
    ...
    bar_full0 = smem.mbarrier(1)
    bar_full1 = smem.mbarrier(1)
    bar_empty = smem.mbarrier(1)
    phase_full0 = reg.scalar(b32, init=0)
    phase_full1 = reg.scalar(b32, init=0)
    phase_empty = reg.scalar(b32, init=0)
    acc = reg.array(f32, 4)

    tid = ptx.special.tid.x()
    with ptx.if_(tid == 0):
        ptx.mbarrier.init(bar_full0[0], 1)
        ptx.mbarrier.init(bar_full1[0], 1)
        ptx.mbarrier.init(bar_empty[0], 128)  # warpgroup-wide
        ptx.fence.proxy_async_shared_cta()

    with ptx.if_(tid == 0):
        ptx.mbarrier.arrive_expect_tx(bar_full0[0], 64*16*2 + 16*8*2)
        ptx.cp.async_.bulk.tensor_2d(dst=sA[0], src=A.tma_desc(), coord=(0, 0), mbar=bar_full0[0])
        ptx.cp.async_.bulk.tensor_2d(dst=sB[0], src=B.tma_desc(), coord=(0, 0), mbar=bar_full0[0])
    ptx.bar.sync(0)
    ptx.mbarrier.wait(bar_full0[0], phase_full0)
    ...
    # ALL 128 threads release the buffer
    ptx.mbarrier.arrive(bar_empty[0])
    # And wait on it (all 128 must arrive before phase flips)
    ptx.mbarrier.wait(bar_empty[0], phase_empty)

    with ptx.if_(tid == 0):
        ptx.mbarrier.arrive_expect_tx(bar_full1[0], 64*16*2 + 16*8*2)
        ptx.cp.async_.bulk.tensor_2d(dst=sA[0], src=A.tma_desc(), coord=(16, 0), mbar=bar_full1[0])
        ptx.cp.async_.bulk.tensor_2d(dst=sB[0], src=B.tma_desc(), coord=(0, 16), mbar=bar_full1[0])
    ptx.bar.sync(0)
    ptx.mbarrier.wait(bar_full1[0], phase_full1)
