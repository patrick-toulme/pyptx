"""K-loop wgmma via jax.jit — with faulthandler traceback dump."""
import faulthandler
import os
import signal
import sys
...
# Dump tracebacks after 20s so we see where the hang is
faulthandler.dump_traceback_later(20, repeat=True, exit=False)

os.environ.setdefault("XLA_FLAGS", "--xla_gpu_enable_command_buffer=")
os.environ["PYPTX_DEBUG"] = "1"
...
print("jax init done", flush=True)
...
print("about to trace PTX", flush=True)
ptx_src = gemm.ptx()
print(f"traced, {len(ptx_src)} bytes", flush=True)

print("calling fn...", flush=True)
...
out = fn(A, B)
print("fn returned, blocking...", flush=True)
out = np.asarray(out)
